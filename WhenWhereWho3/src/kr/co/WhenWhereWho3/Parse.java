...
public class Parse {
	...
	public ArrayList<String> htmlParse(BufferedReader br) {
		...
		try {
			...
			if (br != null) {
				...
				int startIndex = result.indexOf("\"movieTitle\":");
	...
	public ArrayList<Movie> jsonParse(BufferedReader br) {
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				// ex )
				// http://cfile78.uf.daum.net/C110x160/15533D3C4E7933521B928A 에서
				// C110x160부분을
				...
				// ex ) http://cfile79.uf.daum.net/C93x70/1165BC4A4E4DF913184317
				// 에서 C93x70부분을
				// R678x0으로 변환해 주는 작업이 필요하다.
				...
				for (int j = 0; j < 5; j++) {
					...
					content = (JSONObject) resultArray.get(0);
					photo[j] = content.get("content").toString()
							.replace("C93x70", "R678x0");
				}
	...
	public ArrayList<InfoMovieTheater> locationParse(BufferedReader br) {
		...
		try {
			...
			for (int i = 0; i < results.size(); i++) {
				...
				Log.d("구글 플레이스",
						"결과 : "
								+ ((JSONObject) results.get(i)).get("name")
										.toString());

				/*********************************************************************/
				// 위도, 경도 가져오기
				JSONObject geometry = (JSONObject) theater.get("geometry");
				double lat = (double) Double.parseDouble(((JSONObject) geometry
						.get("location")).get("lat").toString());
				Log.d("구글 플레이스", "위도 : " + lat);
				infoMovieTheater.setLat(lat);

				double lng = (double) Double.parseDouble(((JSONObject) geometry
						.get("location")).get("lng").toString());
	...
	public MovieTheater infoParse(BufferedReader br) {
		...
		try {
			...
			// 검색 결과가 없을 경우
			if (object.get("status").toString().equals("INVALID_REQUEST")) {
				Log.d("parse", "검색결과 없음");
				return null;
			}

			// json파일을 읽어온 후 result로 가져옴
			JSONObject results = (JSONObject)object.get("result");
			if( results == null ){
			...
			/*********************************************************************/
			// 영화관 이름 가져오기
			String theaterName = results.get("name").toString();
			Log.d("구글 플레이스", "영화관 이름 : " + theaterName);
			mt.setName(theaterName);
			/*********************************************************************/

			/*********************************************************************/
			// 영화관 주소 가져오기
			String addr = results.get("formatted_address").toString();
			Log.d("구글 플레이스", "영화관 주소 : " + addr);
			mt.setAddr(addr);
			/*********************************************************************/

			/*********************************************************************/
			// 영화관 전화번호 가져오기
			String tel = results.get("international_phone_number").toString();
			Log.d("구글 플레이스", "영화관 전화번호 : " + tel);
			mt.setTel(tel);
			/*********************************************************************/
			br.close();
			return mt;
